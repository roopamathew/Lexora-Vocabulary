(function () {
  ...
  async function syncCurrentStateToCloud() {
    ...
    try {
      ...
      if (
        ...
        await window.LexoraCloud.saveCloudState(
          state
        );
      }
  ...
  function readForm() {
    ...
    return {
      ...
      meaning:
        els.meaning.value.trim(),

      example:
        els.example.value.trim(),
  ...
  function reviewWord(
    ...
    toast(
      `Next review: ${formatDate(item.nextReview)}.`
    );
  }


  function detectSpeechVoice(
    showFallbackMessage
  ) {
    if (
      !(
    ...
    if (
      ...
      showFallbackMessage &&
      !speechFallbackAnnounced
    ) {
      toast(
        "British English voice is not installed. Using the closest available English voice."
      );
  ...
  function speakWord(
    ...
    if (
      ...
      toast(
        "No English speech voice is installed on this device."
      );
    ...
    if (
      ...
      toast(
        "Speech voices are still loading. Try again in a moment."
      );
  ...
  function renderDashboard() {
    ...
    els.wordDayMeaning.textContent =
      dayWord
        ? dayWord.meaning
        : "Your vocabulary journey starts with one useful word.";

    els.wordDayMeta.textContent =
      dayWord
        ? `${dayWord.category} - ${dayWord.difficulty}`
        : "";
  ...
  function getAttentionMessage() {
    if (
      ...
      return (
        "Add words to unlock smart revision prompts."
      );
    }
    ...
    if (
      ...
      return (
        `This word needs your attention: "${candidate.word}" has not been reviewed yet.`
      );
    }

    return (
      `You have not reviewed "${candidate.word}" for ${daysBetween(candidate.lastReviewed, now)} days.`
    );
  }
  ...
  function renderLibrary() {
    ...
    if (!items.length) {
      els.libraryList.innerHTML =
        `<article class="panel"><h2>No vocabulary found</h2><p class="muted">Add a word or adjust your filters.</p></article>`;
    ...
    return `
      <article class="word-card scroll-reveal">
        ...
        <div class="card-meta">
          ...
          <span class="chip">
            Learned ${formatDate(item.dateLearned)}
          </span>
          ...
          <span class="chip">
            Next ${formatDate(item.nextReview)}
          </span>
        ...
        <div class="word-actions">
          ...
          <button
            class="ghost-button small speak-button"
            data-action="speak"
            data-id="${item.id}"
            type="button"
            aria-label="Pronounce ${escapeHtml(item.word)}"
          >
            🔊 Pronounce
          </button>
  ...
  function renderFlashcard() {
    ...
    if (!item) {
      ...
      els.flashBack.innerHTML =
        "<p>Add vocabulary to start flashcards.</p>";
  ...
  function renderCalendar() {
    ...
    for (
      ...
      days.push(
        `<button class="day-cell level-${level}" data-date="${key}" title="${key}: ${total} activity" type="button"></button>`
      );
    }
    ...
    els.calendarDetails.textContent =
      "Select a date to see words learned and reviewed.";
  }
  ...
  function renderWeeklyGraph() {
    ...
    for (
      ...
      days.push(
        `<div class="bar"><span style="height:${height}px"></span><small>${key.slice(5)}</small><strong>${total}</strong></div>`
      );
    }

    els.weeklyGraph.innerHTML =
      days.join("");
  }
  ...
  function earnedBadges() {
    ...
    return [
      ...
      [
        "Professional Vocabulary Master",
        state.words.filter(
          (word) =>
            word.category ===
            "Professional"
    ...
    container.innerHTML =
      badges.length
        ? badges
            .map(
              ([name, earned]) =>
                `<span class="badge ${earned ? "" : "locked"}">${escapeHtml(name)}</span>`
            )
            .join("")
  ...
  function showDailyReminder() {
    ...
    if (
      ...
      if (
        ...
        new Notification(
          "Time to revise today",
          {
            body:
              "Keep your streak alive with one quick review."
          }
        );
  ...
  async function loadOrSyncCloudData() {
    ...
    if (
      ...
      if (useCloud) {
        ...
        toast(
          "Your current vocabulary has been synced to the cloud."
        );
      }
  ...
  async function handleForgotPassword() {
    ...
    try {
      if (
        els.forgotPasswordButton
      ) {
        els.forgotPasswordButton.disabled =
          true;

        els.forgotPasswordButton.textContent =
          "Sending...";
      }
  ...
  async function handlePasswordReset() {
    ...
    try {
      if (
        els.resetPasswordButton
      ) {
        els.resetPasswordButton.disabled =
          true;

        els.resetPasswordButton.textContent =
          "Updating...";
      }
  ...
  function bindEvents() {
      ...
      els.calendarGrid.addEventListener(
        ...
        (event) => {
          ...
          els.calendarDetails.innerHTML =
            `<strong>${cell.dataset.date}</strong><br>` +
            `Learned: ${activity.learned} ` +
            `(${escapeHtml(activity.learnedWords.join(", ") || "none")})<br>` +
            `Reviewed: ${activity.reviewed} ` +
            `(${escapeHtml(activity.reviewedWords.join(", ") || "none")})`;
        }
      );
    }
    ...
    if (
      ...
      backupStorage.addEventListener(
        "click",
        () =>
          exportFile(
            "lexora-backup.json",
    ...
    if (
      ...
      restoreStorage.addEventListener(
        "change",
        (event) =>
          importFile(
            event.target.files[0],
